...
# 视频设置
duration = 60  # 视频持续时间（秒）
fps = 30       # 帧率（帧每秒）
frames = duration * fps  # 动态计算总帧数
output_filename = 'drawing_process_fixed_fps_with_points1.avi'
...
for face in faces:
    ...
    for i in range(17):
        ...
        cv2.circle(face_mask, (x, y), 1, 255, -1)
    ...
    while points_drawn < num_points:
        ...
        if region_mask[y, x] == 255 and edges[y, x] != 0:
            if np.random.rand() < intensity:  # 控制点的密度
                cv2.circle(canvas, (x, y), dot_size, 0, -1)  # 绘制小点
                points_drawn += 1
    return canvas

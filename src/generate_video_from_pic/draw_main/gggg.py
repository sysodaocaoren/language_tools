...
if __name__ == "__main__":
    ...
    for landmarks in face_landmarks:
        ...
        # 绘制右眉毛，从左向右画
        frames['right_eyebrow'] = draw_region(canvas, right_eyebrow, drawn_points, 'right_eyebrow',
                                              frames['right_eyebrow'], edges, 'right')
        print("绘制右眉毛完成")

        # 绘制右眼轮廓，从上往下画
        frames['right_eye_outline'] = draw_region(canvas, right_eye, drawn_points, 'right_eye_outline',
                                                  frames['right_eye_outline'], edges, 'down')
        print("绘制右眼轮廓完成")
    ...
    # 使用膨胀算法绘制面部框外区域
    radius = 0
    while len(remaining_points) > 0 and frames['background'] > 0:
        points_to_draw = []
        ...
        # 逐点绘制
        frame_points = min(len(points_to_draw), int(len(remaining_points) / frames['background']))
        for i in range(frame_points):
    ...
    frame_points = int(total_points / frames['coloring'] * coloring_ratio)
    for i in range(frame_points):

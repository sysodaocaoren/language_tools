...
def get_news_by_keywords(keyword):
    for page_num in (0, total, page):
        ...
        for json_str in response.json().get('data'):
            ...
            if (int(news_dict['comment_count']) == 0):
                continue
            # 新闻入库
            news_data = [news_dict['news_title'], news_dict['abstract'], news_dict['user_id'], TIMESTRF, news_dict['group_id'], "toutiao"]
            newid = db_mysql.insert_news(news_data)
            # 插入搜索
            search_data = [keyword, newid, TIMESTRF]
            db_mysql.insert_search(search_data)
            # 获取评论
            params_pl = params_data_comment(str(newid))
